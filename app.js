...
const apiLeagueIds = { premier: 39, laliga: 140, seriea: 135, bundesliga: 78, ligue1: 61 };
const leaguePalette = { premier: '#16654a', laliga: '#b02743', seriea: '#2372b8', bundesliga: '#d71920', ligue1: '#074f91' };
const liveData = {};
...
function canUseLiveData() { return Boolean(localStorage.getItem('flh-api-key')) || !['', 'localhost', '127.0.0.1'].includes(location.hostname); }
document.querySelectorAll('.league-card').forEach(b => b.addEventListener('click', async () => { fixtureOffset=0; renderLeague(b.dataset.league); if (canUseLiveData() && !liveData[b.dataset.league]) await loadLiveData(b.dataset.league); }));
...
const dialog=$('#settingsDialog'); $('#settingsBtn').addEventListener('click',()=>{ $('#apiKeyInput').value=localStorage.getItem('flh-api-key')||''; dialog.showModal(); }); $('#saveKeyBtn').addEventListener('click',async (e)=>{ e.preventDefault(); const key=$('#apiKeyInput').value.trim(); if(key){ localStorage.setItem('flh-api-key',key); dialog.close(); await loadLiveData(currentLeague); } }); $('#demoBtn').addEventListener('click',()=>{localStorage.removeItem('flh-api-key');Object.keys(liveData).forEach(key=>delete liveData[key]);$('#updatedAt').textContent='Demo data · updated today';renderLeague(currentLeague)}); $('#analystBtn').addEventListener('click',()=>{const d=liveData[currentLeague] || demoData[currentLeague];$('#analystText').textContent=`${d.table[0][0]} remain in control with ${d.table[0][4]} points. The pressure is on ${d.table[1][0]}, who need a result this weekend. Watch ${d.fixtures[0][2]} vs ${d.fixtures[0][3]}—it could reshape the race.`;});
