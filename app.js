...
const breadcrumbRegions = { premier: 'England', laliga: 'Spain', seriea: 'Italy', bundesliga: 'Germany', ligue1: 'France', championsleague: 'Europe', isl: 'India' };
...
const backgroundSources = {
  football: 'assets/background_football.mp4',
  pitch: 'assets/background_pitch.mp4'
};
...
function dashboardStandingClass(key, position) {
  if (key === 'championsleague') {
    if (position <= 8) return 'ucl-direct';
    if (position <= 24) return 'ucl-playoff';
    return 'ucl-eliminated';
  }
  return position <= 4 ? 'ucl' : '';
}
function dashboardLegend(key) {
  if (key === 'championsleague') {
    return '<span class="legend-dot legend-direct"></span>Round of 16 (1-8) <span class="legend-dot legend-playoff"></span>Knockout phase play-offs (9-24) <span class="legend-dot legend-eliminated"></span>Eliminated (25-36)';
  }
  return '<span class="legend-dot legend-direct"></span>Champions League places <span class="legend-dot legend-eliminated"></span>Relegation zone (bottom three)';
}
function renderLeague(key) {
  currentLeague = key;
  const d = liveData[key] || demoData[key];
  if (!d.table.length) return;
  $('#breadcrumbRegion').textContent = breadcrumbRegions[key] || 'Season';
  $('#leagueCountry').textContent = d.country;
  $('#leagueTitle').textContent = d.title;
  $('#fullTableLink').href = `standings.html?league=${key}`;
  $('#standingsBody').innerHTML = d.table.map((club, i) => {
    const position = i + 1;
    return `<tr class="${dashboardStandingClass(key, position)}"><td class="position">${position}</td><td><span class="club"><span class="club-badge" style="background:${club[5]}">${club[1]}</span>${club[0]}</span></td><td>${club[2]}</td><td>${club[3] > 0 ? '+' : ''}${club[3]}</td><td class="points">${club[4]}</td></tr>`;
  }).join('');
  $('#tableNote').innerHTML = dashboardLegend(key);
  const displayed = d.fixtures.slice(fixtureOffset).concat(d.fixtures.slice(0, fixtureOffset));
  $('#fixturesList').innerHTML = displayed.map(f => `<div class="fixture"><time>${f[0]}<br />${f[1]}</time><div class="fixture-teams">${f[2]} <span>vs</span> ${f[3]}</div><span class="fixture-tag">${f[4]}</span></div>`).join('');
  $('#scorersList').innerHTML = d.scorers.map(s => `<li><div><strong>${s[0]}</strong><span class="scorer-team">${s[1]}</span></div><span class="goals">${s[2]}</span></li>`).join('');
  const links = officialLinks[key];
  $('#officialStandings').href = links.standings;
  $('#officialFixtures').href = links.fixtures;
  $('#officialStandings').textContent = `↗ ${d.title} standings`;
  $('#officialFixtures').textContent = `↗ ${d.title} fixtures`;
  document.querySelectorAll('.league-card').forEach(b => b.classList.toggle('selected', b.dataset.league === key));
  $('#matchCount').textContent = d.fixtures.length * 3;
  $('#analystText').textContent = `${d.table[0][0]} lead ${d.title}, but ${d.table[1][0]} are only ${d.table[0][4] - d.table[1][4]} point${d.table[0][4] - d.table[1][4] === 1 ? '' : 's'} back. ${d.fixtures[0][2]} vs ${d.fixtures[0][3]} is the next match to watch.`;
}
...
const dialog=$('#settingsDialog'); $('#settingsBtn').addEventListener('click',()=>dialog.showModal()); $('#saveKeyBtn').addEventListener('click',async (e)=>{ e.preventDefault(); dialog.close(); await loadLiveData(currentLeague); }); $('#demoBtn').addEventListener('click',()=>{Object.keys(liveData).forEach(key=>delete liveData[key]);$('#updatedAt').textContent='Demo data · updated today';renderLeague(currentLeague)}); $('#analystBtn').addEventListener('click',()=>{ location.href = `studio.html?league=${currentLeague}`; });
const redThemeChoice = document.createElement('button');
redThemeChoice.className = 'theme-choice';
redThemeChoice.type = 'button';
redThemeChoice.dataset.themeChoice = 'red';
redThemeChoice.innerHTML = '<span class="theme-swatch red-swatch"></span><span><strong>Red Matchday</strong><small>Bold red matchday energy</small></span>';
document.querySelector('.theme-options').append(redThemeChoice);
...
document.querySelectorAll('[data-background-choice]').forEach(button => button.addEventListener('click', () => setBackground(button.dataset.backgroundChoice)));
setBackground(localStorage.getItem('footballHubBackground') || 'football');
