...
function standingClass(position) {
  if (currentLeague === 'championsleague') {
    if (position <= 8) return 'ucl-direct';
    if (position <= 24) return 'ucl-playoff';
    return 'ucl-eliminated';
  }
  return position <= 4 ? 'europe' : position > allRows.length - 3 ? 'relegation' : '';
}
function renderQualificationLegend() {
  const legend = $('#qualificationLegend');
  if (currentLeague === 'championsleague') {
    legend.innerHTML = '<span class="legend-dot legend-direct"></span>Round of 16 (1-8) <span class="legend-dot legend-playoff"></span>Knockout phase play-offs (9-24) <span class="legend-dot legend-eliminated"></span>Eliminated (25-36)';
  } else {
    legend.innerHTML = '<span class="legend-dot legend-direct"></span>Champions League places <span class="legend-dot legend-eliminated"></span>Relegation zone (bottom three)';
  }
  legend.hidden = false;
}
function renderRows(rows) {
  $('#fullStandings').innerHTML = rows.map(row => `<tr class="${standingClass(row.position)}"><td>${row.position}</td><td><span class="club"><b>${initials(row.team.name)}</b>${row.team.name}</span></td><td>${row.all.played}</td><td>${row.all.win}</td><td>${row.all.draw}</td><td>${row.all.lose}</td><td>${row.goalsDiff > 0 ? '+' : ''}${row.goalsDiff}</td><td class="points">${row.points}</td></tr>`).join('');
